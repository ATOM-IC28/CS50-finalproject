...
class Portfolio:
    ...
    def process_csv(self):
        ...
        n = 0 #it will count the number of times the stocks API fails. (Only this one fails, because it has limited request per hour)
        ...
        for inv in imported_portfolio:
            ...
            if type == "stocks/funds":
                ...
                asset = symbol
                self.check_main_portfolio(asset=asset,quantity=quantity, price=price, type=type)
                ...
                self.check_main_portfolio(asset=symbol,quantity=quantity, price=price, type=type)
    ...
    def save_portfolio(self):
        ...
        if file_name == "": #The empty string happens when the user cancels the selection of a file
            ...
            with open(file_name, "w", newline="", encoding="utf-8") as f:
                fieldnames = ["symbol", "quantity", "price", "value(usd)", "type"]
                writer = csv.DictWriter(f, fieldnames=fieldnames)
                writer.writeheader()
                writer.writerows(data)


    def check_main_portfolio(self, asset, quantity, price, type):

        value = round(price * quantity, 3)

        if self.main_portfolio == []:
            self.main_portfolio.append({"symbol" : asset, "quantity" : quantity, "price" : price, "value(usd)" : value, "type" : type})
            return
    ...
    def add_crypto_to_portfolio(self):
        ...
        if symbol in available_coins:
            asset, price = cmc.get_crypto_price(symbol)
            self.check_main_portfolio(asset=asset,quantity=self.get_amount_to_add(), price=price, type="crypto")
            print()
    ...
    def add_stock_to_portfolio(self):
        ...
        if possible_tickers == None: #For when the API isnt working
            ...
            self.check_main_portfolio(asset=asset,quantity=self.get_shares_to_add(), price=price, type="stocks/funds")
            print()
            self.console.print("Asset successfully added!", style="bold green")
            print()
    ...
    def add_currency_to_portfolio(self):
        ...
        if asset not in available_currencies.keys():
            ...
            self.check_main_portfolio(asset=asset,quantity=self.get_amount_to_add(), price=price, type="currency")
            print()
            self.console.print("Asset successfully added!", style="bold green")
            print()
    ...
    def update_portfolio(self, asset):

        for row in self.main_portfolio:
            if asset.upper() == row["symbol"]:
                quantity = self.get_new_quantity()
        ...
        print()
        self.console.print(f"{asset} is not on your Portfolio.", style="bold red3")
        self.console.print("""If you wish to add it to your Portfolio type "AS" for stocks/funds or "AC" for cryptocurrencies. """, style="bold red3")
        print()
    ...
    def display_portfolio(self):
        ...
        table.add_column("Value(USD)", justify="center", style="green")
        ...
        if  self.main_portfolio == []:
            ...
            for row in self.main_portfolio:
                table.add_row(row["symbol"], str(row["quantity"]), f"""${row["price"]:,.2f}""" , f"""${row["value(usd)"]:,.2f}""")
    ...
    def get_insights(self):
        ...
        if  self.main_portfolio == []:
            ...
            table.add_row("Total Invested", f"""${total_value:,.2f}""")
...
If you wish to use them please switch to an IDE with tkinter support (VScode for example)
    ...
    def command_help(self):
        ...
        self.console.print("U - Update Portfolio ", style="green")
    ...
    def command_update_portfolio(self):
        ...
        while True:
            try:
                asset = str(input("Asset you want to update (i.e. META): ")).strip()
                self.portfolio.update_portfolio(asset)
                break
    ...
    def get_input(self):
        available_commands = ["H","A","S","I","D","U","Q","AS","AC","SS","SC","RM","GI","ACC","SAC"]
        try:
